...
question_body_sha256: 7a886b10f4ba23b87ae9466ca11988249dc7dbb4d14590f2ca35b6bf01d0c6a9
...
(1) = 0, g''(x) = 2 + \lambda (2 + \frac{1}{x^2}), g'' (1) = 0$ ，所以 $\lambda = -\frac{2}{3}$ 所以 $g(x) = x^{2} - 2x - \frac{2}{3}(x^{2} - x - \ln x), g'(x) = 2x - 2 - \frac{2}{3}(2x - 1 - \frac{1}{x}) = \frac{2}{3}(x + \frac{1}{x}) - \frac{4}{3} \geqslant 0$ 所以 $g(x)$ 单调递增，所以 $g(x_{2}) > g(x_{1})$ ，即 $x_{2}^{2} - 2x_{2} - \frac{2}{3}(x_{2}^{2} - x_{2} - \ln x_{2}) > x_{1}^{2} - 2x_{1} - \frac{2}{3}(x_{1}^{2} - x_{1} - \ln x_{1})$ ，所以 $x_{2}^{2} - 2x_{2} > x_{1}^{2} - 2x_{1}$ ，即 $x_{1} + x_{2} > 2$ 得证！

![](../../../../images/questions/part-001/1060909b95bf26d90be31b013303f51303aaf107054e08ebf702ed8f32e6eb7c.jpg)

我们的本意是得到 $x_{1} + x_{2} > x_{1}^{\prime} + x_{2}^{\prime} = 2$ ，故需要拟合一个比原函数(绿色)先小后大的共极值点二次函数(红色)，但是倍数该如何调整呢？我们发现做差之后， $h(x) = m(x) - \lambda (x^{2} - 2x + 1)$ 是越来越小的，即 $h(x)$ 单调递减，由共极值点可知 $h^{\prime}
(1) = 0$ ，故1不为函数极值点，而是拐点的横坐标，即 $h'' (1) = 0$ 从而锁定参数.
...
$h'
(1)$ ，所以 $\frac{x_{1}+x_{2}}{2}>1$ ，所以 $x_{1}+x_{2}>2$ 得证！
...
(1)由 $g(x) = x^{2}\ln x - ax^{2} + x$ ，有 $g'(x) = 2x\ln x + x - 2ax + 1$
...
法二：（单调性调整）令 $\frac{1}{x}=t$ ，则 $a=t-\ln t,\quad g(t)=t^{2}-2t+\lambda(t-\ln t),\quad g'(t)=2t-2+\lambda\left(1-\frac{1}{t}\right)$ ， $g''(t)=2+\frac{\lambda}{t^{2}},\quad g''
(1)=2+\lambda=0\Rightarrow\lambda=-2$ ，此时 $g'(t)=2t-4+\frac{2}{t}\geqslant0$ 恒成立， $g(t)$ 单调递增，
故 $g(t_{2})>g(t_{1})\Leftrightarrow t_{2}^{2}-2t_{2}>t_{1}^{2}-2t_{1}\Rightarrow t_{1}+t_{2}>2$ ，故 $x_{1}+x_{2}>2x_{1}x_{2}$ 得证！
<!-- question-source:end -->

![[高中/总复习/专题/2026版高中《mst老唐说题》导数专题（数学）/下册/第09章_导数与双变量/第二节_极值点偏移/四_单调性调整与阿达玛积分不等式/例题/answers/Q00047446A1.md]]
