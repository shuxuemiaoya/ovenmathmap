---
answer_for: "Q00052860"
answer_provenance: authoritative
answer_source_kind: "worked-example"
answer_source_body_sha256: c6704c26a339882391885945b2450f05b9bfaed9809c33a37613191602966131
---
> [!faq]- 《导数专题(下册)》例题解析
>
> > [!success]- **【答案】** 详见解析
...
> >
> > 因为 $f\left(\frac{1}{e}\right)=0$ ， $f(1)=1$ ，且当 $x\to+\infty$ 时， $f(x)\to0$ ，所以函数 $f(x)$ 的图像如下所示：
> >
...
> >
> > (ii）不妨设 $x_{1}<x_{2}$ ，则 $\frac{1}{\mathrm{e}}<x_{1}<1<x_{2}$ ，由(1)知： $f(x_{1})>\frac{\mathrm{e}x_{1}-1}{\mathrm{e}-1}$ ，
> >
...
> >
> > 于是构造函数 $h(x)=\ln x+\frac{1}{2}x^{3}-x^{2}-\frac{1}{2}x+1,\quad x>1$ ，所以 $h'(x)=\frac{1}{x}+\frac{3}{2}x^{2}-2x-\frac{1}{2}=\frac{(x-1)^{2}(3x+2)}{2x}>0$ ，所以 $h(x)$ 在 $(1,+\infty)$ 上为增函数，所以 $h(x)>h(1)=0$ ，所以 $h(x_{2})>0$ 成立，所以 $\left|x_{1}-x_{2}\right|>\sqrt{2(1-a)}+(1-a)\left(1-\frac{1}{e}\right)$ 成立.
> >
