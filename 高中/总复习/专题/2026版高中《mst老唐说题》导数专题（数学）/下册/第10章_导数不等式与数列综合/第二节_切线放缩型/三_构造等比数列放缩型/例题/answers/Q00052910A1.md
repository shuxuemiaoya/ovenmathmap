---
answer_for: "Q00052910"
answer_provenance: authoritative
answer_source_kind: "worked-example"
answer_source_body_sha256: e5b9cf75815ae5602fdec5a18fbd5674ae8c819c9ab60fe9ea17932b1ef54a95
---
> [!faq]- 《导数专题(下册)》例题解析
>
> > [!success]- **【答案】** 详见解析
>
> > [!note]- **【解析】**
> > 解析 (1)由 $f'(x)=e^{x}-2a$ 且 $f(0)=0$ ，令 $f'(0)=0\Rightarrow a=\frac{1}{2}$ ，当 $a=\frac{1}{2}$ 时，则 $f(x)=e^{x}-x-1$ ， $f'(x)=e^{x}-1$ ，当 $f'(x)>0$ ，有x>0，即 $f(x)$ 在 $(0,+\infty)$ 上递增；当 $f'(x)<0$ ，有x<0，即 $f(x)$ 在 $(-∞,0)$ 上递减；
> >
...
> > 所以函数 $g(x)$ 在 $(-∞,1)$ 递减， $(1,+∞)$ 递增，所以 $x≥0$ 时 $g(x)\geqslant g(1)=0$ 成立。若 $e-2≥a>0$ ，则 $x≥0$ 时 $g'(x)=(x-1)(e^{x}-2a)$ ， $g'(x)=0$ 得 x=1 或 $x=\ln(2a)$
> >
