---
answer_for: "Q00044634"
answer_provenance: authoritative
answer_source_kind: "separate-authoritative"
answer_source_body_sha256: a5eb3828757217f1880988f778d3cb8159c520859e603345f7f3d4ade9997808
---
> [!faq]- 《专题4    函数的最值(值域)-函数》例题解析
>
> > [!success]- **【答案】** 详见解析
>
> > [!note]- **【分析】**
> > 本题未单列分析。
>
> > [!note]- **【解析】**
> > (1)当 $a = \frac{1}{2}$ 时，用定义证明函数的单调性并求函数 $f(x)$ 的最小值；
> >
> > (2)若对任意 $x \in [1, +\infty)$ , $f(x) > 0$ 恒成立, 试求实数 $a$ 的取值范围.
> >
> > 4. 解析
> > (1) 当 $a = \frac{1}{2}$ 时, $f(x) = x + \frac{1}{2x} + 2$ , 任取 $1 \leq x_1 < x_2$ ,
> >
> > $$
> > f \left(x _ {1}\right) - f \left(x _ {2}\right) = \left(x _ {1} - x _ {2}\right) + \left(\frac {1}{2 x _ {1}} - \frac {1}{2 x _ {2}}\right) = \frac {\left(x _ {1} - x _ {2}\right) (2 x _ {1} x _ {2} - 1)}{2 x _ {1} x _ {2}}
> > $$
> >
> > 因为 $1 \leq x_{1} < x_{2}$ ，所以 $x_{1}x_{2} > 1$ ，所以 $2x_{1}x_{2} - 1 > 0$ 。又 $x_{1} - x_{2} < 0$ ，所以 $f(x_{1}) < f(x_{2})$ ，
> >
> > 所以 $f(x)$ 在 $[1, +\infty)$ 上是增函数，所以 $f(x)$ 在 $[1, +\infty)$ 上的最小值为 $f
> > (1)=\frac{7}{2}$ .
> >
> > (2)因为在区间 $[1, +\infty)$ 上， $f(x) = \frac{x^2 + 2x + a}{x} > 0$ 恒成立，则 $\left\{ \begin{array}{l} x^2 + 2x + a > 0, \\ x \geq 1 \end{array} \right. \Leftrightarrow \left\{ \begin{array}{l} a > - (x^2 + 2x), \\ x \geq 1, \end{array} \right.$
> >
> > 等价于 a 大于函数 $\varphi(x)=-(x^{2}+2x)$ 在 $[1,+\infty)$ 上的最大值.
> >
> > 因为 $\varphi (x) = -(x + 1)^2 +1$ 在 $[1, + \infty)$ 上单调递减，
> >
> > 所以当 x=1 时， $\varphi(x)$ 取最大值为 $\varphi
> > (1)=-3$ ，所以 a>-3，
> >
> > 故实数 a 的取值范围是 $(-3, +\infty)$ .
