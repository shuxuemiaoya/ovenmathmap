---
answer_for: "Q00042657"
answer_provenance: authoritative
answer_source_kind: "separate-authoritative"
answer_source_body_sha256: 211f3c465482efd21ca39e1a935cd5798b05a8c00f397c67b52477a2ebe8c04f
---
> [!faq]- 《专题29  距离型、参数型、数量积型及四边形面积型最值问题-圆锥曲线》例题解析
>
> > [!success]- **【答案】** 详见解析
>
> > [!note]- **【分析】**
> > 本题未单列分析。
>
> > [!note]- **【解析】**
> > (1)求直线 $AP$ 斜率的取值范围;
> >
> > (2)求 $|PA|\cdot|PQ|$ 的最大值.
> >
> > ![](../../../images/questions/f07953a8e6eb2b0019eda8a6128682ae73641992721569458934033be3c12c2b.jpg)
> >
> > 4. 解析
> > (1)设直线 $AP$ 的斜率为 $k$ , $k = \frac{x^2 - \frac{1}{4}}{x + \frac{1}{2}} = x - \frac{1}{2}$ , 因为 $-\frac{1}{2} < x < \frac{3}{2}$ , 所以 $-1 < x - \frac{1}{2} < 1$ ,
> >
> > 即直线 AP 斜率的取值范围是 $(-1, 1)$ .
> >
> > (2)设直线 $AP$ 的斜率为 $k$ . 则直线 $AP$ 的方程为 $y - \frac{1}{4} = k\left(x + \frac{1}{2}\right)$ , 即 $kx - y + \frac{1}{2} k + \frac{1}{4} = 0$ ,
> >
> > 因为直线 BQ 与直线 AP 垂直，所以直线 BQ 的方程为 $x + ky - \frac{9}{4}k - \frac{3}{2} = 0$ ，
> >
> > 联立 $\left\{\begin{aligned}kx-y+\frac{1}{2}k+\frac{1}{4}=0,\\ x+ky-\frac{9}{4}k-\frac{3}{2}=0,\end{aligned}\right.$ 解得点Q的横坐标 $x_{Q}=\frac{-k^{2}+4k+3}{2(k^{2}+1)}$ .
> >
> > $$
> > | P A | = \sqrt {1 + k ^ {2}} \left(x + \frac {1}{2}\right) = \sqrt {1 + k ^ {2}} (k + 1), | P Q | = \sqrt {1 + k ^ {2}} \left(x _ {Q} - x\right) = - \frac {(k - 1) (k + 1) ^ {2}}{\sqrt {k ^ {2} + 1}},
> > $$
> >
> > 所以 $|PA|\cdot|PQ|=-(k-1)(k+1)^{3}$ .
> >
> > 令 $f(k) = -(k-1)(k+1)^{3}$ ，因为 $f'(k) = -(4k-2)(k+1)^{2}$ ，令 $f(k) = 0$ ，得 $k = \frac{1}{2}$ 或 k = -1（舍），
> >
> > 所以 $f(k)$ 在区间 $\left(-1, \frac{1}{2}\right)$ 上单调递增， $\left(\frac{1}{2}, 1\right)$ 上单调递减，因此当 $k = \frac{1}{2}$ 时， $|PA| \cdot |PQ|$ 取得最大值 $\frac{27}{16}$ .
