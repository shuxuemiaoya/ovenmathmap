---
answer_for: "Q00043061"
answer_provenance: authoritative
answer_source_kind: "separate-authoritative"
answer_source_body_sha256: abed869b05c202929e6d15482b14e7ea1d88de23e67ffc88a3ca05ad9aa7a445
---
> [!faq]- 《专题09 函数的最值-导数》例题解析
>
> > [!success]- **【答案】** 详见解析
>
> > [!note]- **【分析】**
> > 本题未单列分析。
>
> > [!note]- **【解析】**
> > (1)讨论 $f(x)$ 的单调性；
> >
> > (2)当 $0 < a < 3$ 时，记 $f(x)$ 在区间 $[0, 1]$ 的最大值为 $M$ ，最小值为 $m$ ，求 $M - m$ 的取值范围.
> >
> > 13. 解析
> > (1) $f(x)$ 的定义域为 R, $f'(x)=6x^{2}-2ax=2x(3x-a)$ . 令 $f'(x)=0$ , 得 x=0 或 $x=\frac{a}{3}$ .
> >
> > 若 a>0，则当 $x\in(-\infty,0)\cup\left(\frac{a}{3},+\infty\right)$ 时， $f'(x)>0$ ，当 $x\in\left(0,\frac{a}{3}\right)$ 时， $f'(x)<0$ ，
> >
> > 故 $f(x)$ 在 $(-\infty, 0)$ , $\left[\frac{a}{3}, +\infty\right]$ 上单调递增, 在 $\left[0, \frac{a}{3}\right]$ 上单调递减;
> >
> > 若 a=0，则 $f(x)$ 在 $(-∞, +∞)$ 上单调递增；
> >
> > 若 a<0，则当 $x\in\left(-\infty,\frac{a}{3}\right)\cup(0,+\infty)$ 时， $f(x)>0$ ，当 $x\in\left(\frac{a}{3},0\right)$ 时， $f(x)<0$ ，
> >
> > 故 $f(x)$ 在 $\left[-\infty, \frac{a}{3}\right]$ , $(0, +\infty)$ 上单调递增, 在 $\left[\frac{a}{3}, 0\right]$ 上单调递减.
> >
> > (2)当 $0 < a < 3$ 时，由
> > (1)知， $f(x)$ 在 $\left[0, \frac{a}{3}\right]$ 上单调递减，在 $\left[\frac{a}{3}, 1\right]$ 上单调递增，所以 $f(x)$ 在 $[0, 1]$ 的最小值为 $f\left(\frac{a}{3}\right) = -\frac{a^3}{27} + 2$ ，最大值为 $f(0) = 2$ 或 $f (1) = 4 - a$ .
> >
> > 于是 $m = -\frac{a^3}{27} + 2, M = \begin{cases} 4 - a, & 0 < a < 2, \\ 2, & 2 \leq a < 3. \end{cases}$ 所以 $M - m = \begin{cases} 2 - a + \frac{a^3}{27}, & 0 < a < 2, \\ \frac{a^3}{27}, & 2 \leq a < 3. \end{cases}$
> > - **①**：当 0<a<2 时，可知 $y=2-a+\frac{a^{3}}{27}$ 单调递减，所以 M-m 的取值范围是 $\left(\frac{8}{27}, 2\right)$ .
> > - **②**：当 $2 \leq a < 3$ 时， $y = \frac{a^{3}}{27}$ 单调递增，所以 M - m 的取值范围是 $\left[\frac{8}{27}, 1\right]$ .
> > 综上，M-m 的取值范围是 $\left[\frac{8}{27}, 2\right]$ .
