---
answer_for: "Q00053123"
answer_provenance: authoritative
answer_source_kind: "worked-example"
answer_source_body_sha256: 22ae2c3ce40b1c1bac19e4469c00ea16a2da88fee8269d11d6305abb9c27fcf2
---
> [!faq]- 《圆锥曲线专题(下册)》例题解析
>
> > [!success]- **【答案】** 详见解析
...
> >
> > (3)根据旋转后平行性质不变，故需要证明 $P_{n}^{\prime}P_{n + 3}^{\prime}\parallel P_{n + 1}^{\prime}P_{n + 2}^{\prime}$ ，由(2)得， $P_{\mathrm{i}}^{\prime}\left(x_{\mathrm{i}},\frac{9}{2x_{\mathrm{i}}}\right),P_{j}^{\prime}\left(x_{j},$ $\frac{9}{2x_j}\bigg),k' = k_{P_n'P_j'} = -\frac{9}{2x_i'x_j'}$ ，反比例函数任意两点的对合式方程为： $\frac{2x_{\mathrm{i}}x_{j}}{9} y + x = x_{\mathrm{i}} + x_{j}$ 所以 $k_{P_n'P_n' + 3} = -\frac{9}{2x_{n + 3}^{\prime}x_{n}^{\prime}},k_{P_{n + 1}^{\prime}P_{n + 2}^{\prime}} = -\frac{9}{2x_{n + 2}^{\prime}x_{n + 1}^{\prime}}$ ，由于 $\{x_n^\prime \}$ 是等比数列，故 $x_{n}^{\prime}x_{n + 3}^{\prime} = x_{n + 1}^{\prime}x_{n + 2}^{\prime}$ 故 $k_{P_n'P_n' + 3} = k_{P_{n + 1}^{\prime}P_{n + 2}^{\prime}}.$
> >
