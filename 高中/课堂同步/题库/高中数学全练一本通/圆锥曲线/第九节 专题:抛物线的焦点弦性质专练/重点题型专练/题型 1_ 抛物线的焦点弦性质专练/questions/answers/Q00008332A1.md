...
> [!faq]- 全练一本通解析
> **【答案】** （1）2p （2） $x_{1}+x_{2}+p$ （3） $\frac{p^{2}}{4}$ ; $-p^{2}$ （4）相切（5） $\frac{p}{1-\cos\alpha}$ ; $\frac{p}{1+\cos\alpha}$ ; $\frac{2p}{\sin^{2}\alpha}$ ; $\frac{2}{p}$ （6）y 轴
>
> **【解析】**
> （1）焦点坐标为 $\left(\frac{p}{2},0\right)$ ，所以当 $x=\frac{p}{2}$ 时， $y^{2}=2p\times\frac{p}{2}=p^{2}\Rightarrow\left|y\right|=p$ ，所以通径为 2p，
