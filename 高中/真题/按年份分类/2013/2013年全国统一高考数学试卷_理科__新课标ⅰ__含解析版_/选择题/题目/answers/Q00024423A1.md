...
>
> > [!note]- **【解析】**
> > 分析】（Ⅰ）取 AB 的中点 O，连接 OC， $OA_{1}$ ， $A_{1}B$ ，由已知可证 $OA_{1} \perp AB$ ， $AB \perp$ 平面
> >
> > $OA_{1}C$ ，进而可得 $AB \perp A_{1}C$ ;
> >
> > （II）易证OA， $\mathrm{OA}_1$ ，OC两两垂直.以O为坐标原点， $\overrightarrow{\mathrm{OA}}$ 的方向为x轴的正向， $|\overrightarrow{\mathrm{OA}}$
> >
> > |为单位长，建立坐标系，可得 $\overrightarrow{BC}$ ， $\overrightarrow{BB_1}$ ， $\overrightarrow{A_1C}$ 的坐标，设 $\vec{n} = (x, y, z)$ 为平面
> >
> > $\mathrm{BB}_{1}\mathrm{C}_{1}\mathrm{C}$ 的法向量，则 $\left\{ \begin{array}{l} \overrightarrow{\mathrm{n}} \cdot \overrightarrow{\mathrm{BC}} = 0 \\ \overrightarrow{\mathrm{n}} \cdot \overrightarrow{\mathrm{BB}_{1}} = 0 \end{array} \right.$ ，可解得 $\vec{\mathrm{n}} = (\sqrt{3}, 1, -1)$ ，可求 $|\cos < \vec{\mathrm{n}}, \overrightarrow{\mathrm{A}_{1}C}$
> >
> > > |，即为所求正弦值.
> >
> > 【
> > 解答】解：（Ⅰ）取 AB 的中点 O，连接 OC， $OA_{1}$ ， $A_{1}B$ ，
> >
