...
answer_source_body_sha256: 918f5e206394f349acf4ccaacf9d51f9d17250171218f2efa45f1dc4d0ef5622
---
...
>
> > [!note]- **【解析】**
> > 分析】对于 A，对于三次函数 $f(x)=x^{3}+ax^{2}+bx+c$ ，由于当 $x\to-\infty$ 时， $y\to-\infty$ ，当
> >
> > $x \rightarrow +\infty$ 时， $y \rightarrow +\infty$ ，故在区间 $(-∞, +∞)$ 肯定存在零点；
> >
> > 对于 B，根据对称变换法则，求出对应中心坐标，可以判断；
> >
> > 对于 C: 采用取特殊函数的方法, 若取 a = -1, b = -1, c = 0, 则 $f(x) = x^{3} - x^{2} - x$ , 利用
> >
> > 导数研究其极值和单调性进行判断；
> >
> > D: 若 $x_{0}$ 是 $f(x)$ 的极值点，根据导数的意义，则 $f'(x_{0}) = 0$ ，正确.
> >
> > 【
> > 解答】解：
> >
...
> >
> > 【点评】本题考查了导数在求函数极值中的应用, 利用导数求函数的单调区间, 及
> > 导数的运算.
