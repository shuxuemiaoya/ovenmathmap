...
>
> > [!note]- **【解析】**
> > 分析】设 AB=1，则 $AA_{1}=2$ ，分别以 $\overrightarrow{D_{1}A_{1}}$ 、 $\overrightarrow{D_{1}C_{1}}$ 、 $\overrightarrow{D_{1}D}$ 的方向为 x 轴、y 轴、z 轴的正方向建立空间直角坐标系，设 $\vec{n}=(x,y,z)$ 为平面 $BDC_{1}$ 的一个法向量，CD 与平面 $BDC_{1}$ 所成角为 $\theta$ ，
> >
