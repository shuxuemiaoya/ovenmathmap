...
>
> > [!note]- **【解析】**
> > 分析】由已知当 x>0 时总有 $xf'(x)-f(x)<0$ 成立，可判断函数 $g(x)=\frac{f(x)}{x}$ 为减函数，由已知 $f(x)$ 是定义在 R 上的奇函数，可证明 $g(x)$ 为 $(-∞,0)∪(0,+∞)$ 上的偶函数，根据函数 $g(x)$ 在 $(0,+∞)$ 上的单调性和奇偶性，模拟 $g(x)$ 的图象，而不等式 $f(x)>0$ 等价于 $x•g(x)>0$ ，数形结合解不等式组即可.
> >
> > 【
> > 解答】解：设 $g(x) = \frac{f(x)}{x}$ ，则 $g(x)$ 的导数为： $g'(x) = \frac{x f'(x) - f(x)}{x^2}$ ， $\because$ 当 $x > 0$ 时总有 $x f'(x) < f(x)$ 成立，  
