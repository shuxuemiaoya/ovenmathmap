...
>
> > [!note]- **【解析】**
> > 分析: (I) 连接 AO, A₁D, 根据几何体的性质得出 A₁O⊥A₁D, A₁D⊥BC, 利用直线平面的垂直定理判断.
> >
> > (II) 利用空间向量的垂直得出平面 $BB_{1}C_{1}C$ 的法向量 $\overrightarrow{n}=(\sqrt{7},0,1)$ ，|根据与 $\overrightarrow{BA_{1}}$ 数量积求解余弦值，即可得出直线 $A_{1}B$ 和平面 $BB_{1}C_{1}C$ 所成的角的正弦值.
> >
> > 解答：证明：（I）∵AB=AC=2，D是 $B_{1}C_{1}$ 的中点.
> >
> > $$
