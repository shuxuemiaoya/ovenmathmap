---
answer_for: "Q00021165"
answer_provenance: authoritative
answer_source_kind: embedded-exam-solution
answer_value_source: "explicit-answer"
answer_source_body_sha256: dda712abc2b689179225ca1f8e89e2048e59972255926af6e6e3b655e4741d2c
---
...
>
> > [!note]- **【分析】**
> > 由 $x_{1}, x_{2}$ 分别是函数 $f(x) = 2a^{x} - \mathrm{e}x^{2}$ 的极小值点和极大值点，可得 $x \in (-\infty, x_{1}) \cup (x_{2}, +\infty)$ 时， $f'(x) < 0$ ， $x \in (x_{1}, x_{2})$ 时， $f'(x) > 0$ ，再分 $a > 1$ 和 $0 < a < 1$ 两种情况讨论，方程 $2\ln a \cdot a^{x} - 2\mathrm{e}x = 0$ 的两个根为 $x_{1}, x_{2}$ ，即函数 $y = \ln a \cdot a^{x}$ 函数 $y = \mathrm{e}x$ 的图象有两个不同的交点，构造函数 $g(x) = \ln a \cdot a^x$ ，根据导数的结合意义结合图象即可得出答案.
>
> > [!note]- **【解析】**
