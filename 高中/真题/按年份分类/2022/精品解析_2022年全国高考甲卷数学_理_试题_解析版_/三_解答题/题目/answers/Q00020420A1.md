---
answer_for: "Q00020420"
answer_provenance: authoritative
answer_source_kind: embedded-exam-solution
answer_value_source: "explicit-answer"
answer_source_body_sha256: 92454e3097f0e1bd874f0addd8c9410ca14301eea152f987341b52f1e016d1ff
---
...
> >
> > (2) 利用分析法，转化要证明条件 $\frac{\mathrm{e}^x}{x} - x\mathrm{e}^{\frac{1}{x}} - 2\left[\ln x - \frac{1}{2}\left(x - \frac{1}{x}\right)\right] > 0$ ，再利用导数即可得证.
>
> > [!note]- **【解析】**
> > 【小问 1 详解】
> >
...
> >
> > 当 $x \in (1, +\infty), f'(x) > 0, f(x)$ 单调递增 $f(x) \geq f(1) = e + 1 - a$
> >
...
> >
> > $$
> > \varphi (x) = \frac {\mathrm{e} ^ {x}}{x} (x > 1), \varphi^ {\prime} (x) = \left(\frac {1}{x} - \frac {1}{x ^ {2}}\right) \mathrm{e} ^ {x} = \frac {x - 1}{x ^ {2}} \mathrm{e} ^ {x} > 0
> > $$
> >
> > 所以 $\varphi (x) > \varphi (1) = \mathrm{e}$ 而 $\mathrm{e}^{\frac{1}{x}} <   \mathrm{e}$
> >
