...
>
> > [!note]- **【解析】**
> > 分析】（I）利用勾股定理的逆定理证明 $AB_{1} \perp A_{1}B_{1}$ ， $AB_{1} \perp B_{1}C_{1}$ ，从而可得
> >
> > $AB_{1}$ ⊥ 平面 $A_{1}B_{1}C_{1}$ ;
> >
> > (Ⅱ) 以 AC 的中点为坐标原点建立空间坐标系，求出平面 $ABB_{1}$ 的法向量 $\vec{n}$ ，计
> >
> > 算 $\vec{n}$ 与 $\overrightarrow{AC_{1}}$ 的夹角即可得出线面角的大小.
> >
> > 【
> > 解答】（Ⅰ）证明：∵A₁A⊥平面ABC，B₁B⊥平面ABC，
> >
