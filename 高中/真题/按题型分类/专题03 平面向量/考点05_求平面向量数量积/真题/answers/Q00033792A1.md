...
>
> > [!note]- **【分析】**
> > 【分析】可得 $\angle BAD = 120^{\circ}$ ，利用平面向量数量积的定义求得 $\lambda$ 的值，然后以点 $B$ 为坐标原点， $BC$ 所在直线为 $x$ 轴建立平面直角坐标系，设点 $M(x,0)$ ，则点 $N(x + 1,0)$ （其中 $0 \leq x \leq 5$ ），得出 $\overline{DM} \cdot \overline{DN}$ 关于 $x$ 的函数表达式，利用二次函数的基本性质求得 $\overline{DM} \cdot \overline{DN}$ 的最小值。
>
> > [!note]- **【解析】**
> > 【详解】∵ $AD = \lambda BC$ ，∴ $AD \parallel BC$ ，∴ $\angle BAD = 180^{\circ} - \angle B = 120^{\circ}$
> > $$
