...
>
> > [!note]- **【分析】**
> > 解法一：根据台体的体积公式可得三棱台的高 $h = \frac{4\sqrt{3}}{3}$ ，做辅助线，结合正三棱台的结构特征求得 $AM = \frac{4\sqrt{3}}{3}$ ，进而根据线面夹角的定义分析求解；解法二：将正三棱台 $ABC - A_{1}B_{1}C_{1}$ 补成正三棱锥 $P - ABC$ ， $A_{1}A$ 与平面 $ABC$ 所成角即为 $PA$ 与平面 $ABC$ 所成角，根据比例关系可得 $V_{P - ABC} = 18$ ，进而可求正三棱锥 $P - ABC$ 的高，即可得结果·
>
> > [!note]- **【解析】**
> > 解法一：分别取 $BC, B_{1}C_{1}$ 的中点 $D, D_{1}$ ，则 $AD = 3\sqrt{3}, A_{1}D_{1} = \sqrt{3}$
> >
