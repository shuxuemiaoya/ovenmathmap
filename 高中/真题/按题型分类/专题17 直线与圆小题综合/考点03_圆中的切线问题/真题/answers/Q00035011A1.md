...
>
> > [!note]- **【分析】**
> > 【分析】方法一：根据切线的性质求切线长，结合倍角公式运算求解；方法二：根据切线的性质求切线长结合余弦定理运算求解；方法三：根据切线结合点到直线的距离公式可得 $k^{2}+8k+1=0$ ，利用韦达定理结合夹角公式运算求解。
>
> > [!note]- **【解析】**
> > 【详解】方法一：因为 $x^{2} + y^{2} - 4x - 1 = 0$ ，即 $(x - 2)^{2} + y^{2} = 5$ ，可得圆心 $C(2,0)$ ，半径 $r = \sqrt{5}$
