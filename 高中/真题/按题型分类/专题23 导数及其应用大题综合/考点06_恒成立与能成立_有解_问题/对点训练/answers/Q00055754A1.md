---
answer_for: "Q00055754"
answer_provenance: authoritative
answer_source_kind: "separate-authoritative"
answer_source_body_sha256: 5afeea35b1adcbcccc3af88825375e21f49f264db24a25398af32fe79132e44c
---
...
> >
> > 当 $x \in (1, +\infty), f'(x) > 0, f(x)$ 单调递增 $f(x) = f
> > (1) = e + 1 - a$
> >
...
> >
> > $$
> > \varphi (x) = \frac {e ^ {x}}{x} (x > 1), \varphi^ {\prime} (x) = \left(\frac {1}{x} - \frac {1}{x ^ {2}}\right) e ^ {x} = \frac {x - 1}{x ^ {2}} e ^ {x} > 0
> > $$
> >
> > 所以 $\varphi(x) > \varphi
> > (1) = e$ ，而 $e^{\frac{1}{x}} < e$
...
> >
> > 即 $g(x) > g
> > (1) = 0$ ，所以 $\frac{e^{x}}{x} - xe^{\frac{1}{x}} > 0$
> >
> > 令 $h(x) = \ln x - \frac{1}{2}\left(x - \frac{1}{x}\right), x > 1$
> >
> > $h^{\prime}(x) = \frac{1}{x} -\frac{1}{2}\left(1 + \frac{1}{x^{2}}\right) = \frac{2x - x^{2} - 1}{2x^{2}} = \frac{-(x - 1)^{2}}{2x^{2}} < 0$
> >
> > 所以 $h(x)$ 在 $(1,+\infty)$ 单调递减
> >
> > 即 $h(x) < h
> > (1) = 0$ ，所以 $\ln x - \frac{1}{2}\left(x - \frac{1}{x}\right) < 0$ ；
> >
...
> > $$
> >
> > 不妨设 $t = \sqrt{\frac{x_1}{x_2}} > 1$ ，则只需证 $2\ln t < t - \frac{1}{t}$
> >
