...
>
> > [!note]- **【解析】**
> > 【答案】（Ⅰ） $EF // B_{1}C$ ；（Ⅱ） $\frac{\sqrt{6}}{3}$
> > 【详解】试题分析：（Ⅰ）证明：依据正方形的性质可知 $A_{1}B_{1} // AB // DC$ ，且 $A_{1}B_{1} = AB = DC$ ，，从而 $A_{1}B_{1}CD$ 为平行四边形，则 $B_{1}C // A_{1}D$ ，根据线面平行的判定定理知 $B_{1}C // \text{面 } A_{1}DE$ ，再由线面平行的性质定理知 $EF // B_{1}C \cdot (\text{Ⅱ})$ 因为四边形 $AA_{1}B_{1}B$ ， $ADD_{1}A_{1}$ ， $ABCD$ 均为正方形，所以
