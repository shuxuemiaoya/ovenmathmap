...
>
> > [!success]- **【答案】** （Ⅰ）证明见解析；（Ⅱ） $\frac{\sqrt{6}}{3}$ 【详解】试题分析：（Ⅰ）先证 $BE \perp OA_{1}$ ， $BE \perp OC$ ，再可证 $BE \perp$ 平面 $A_{1}OC$ ，进而可证 $CD \perp$ 平面 $A_{1}OC$ （Ⅱ）先建立空间直角坐标系，再算出平面 $\mathrm{A}_{1} \mathrm{BC}$ 和平面 $\mathrm{A}_{1} \mathrm{CD}$ 的法向量，进而可得平面 $\mathrm{A}_{1} \mathrm{BC}$ 与平面 $\mathrm{A}_{1} \mathrm{CD}$ 夹角的余弦值。 试题
>
> > [!note]- **【分析】**
> > 本题未单列分析。
>
