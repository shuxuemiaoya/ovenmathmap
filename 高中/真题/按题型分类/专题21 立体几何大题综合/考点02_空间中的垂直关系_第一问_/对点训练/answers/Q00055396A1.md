---
answer_for: "Q00055396"
answer_provenance: authoritative
answer_source_kind: "separate-authoritative"
answer_source_body_sha256: 86ca04f79531c3e791b95dbf6e196f5499ff4754b5d7a2a276b7dca858f1da51
---
...
>
> > [!note]- **【分析】**
> > （Ⅰ）方法一：作 $DH \perp AC$ 交 $AC$ 于 $H$ ，连接 $BH$ ，由题意可知 $DH \perp$ 平面 $ABC$ ，即有 $DH \perp BC$ 根据勾股定理可证得 $BC \perp BH$ ，又 $EF // BC$ ，可得 $DH \perp EF$ ， $BH \perp EF$ ，即得 $EF \perp$ 平面 $BHD$ ，即证得 $EF \perp DB$ ；
> >
> > (Ⅱ) 方法一：由 $DF // CH$ ，所以 $DF$ 与平面 $DBC$ 所成角即为 $CH$ 与平面 $DBC$ 所成角，作 $HG \perp BD$ 于 $G$
> >
> > 连接 $CG$ ，即可知 $\angle HCG$ 即为所求角，再解三角形即可求出 $DF$ 与平面 $DBC$ 所成角的正弦值.
>
> > [!note]- **【解析】**
> > （I）[方法一]：几何证法
> >
...
> >
> > $$
> > \therefore B D = \left(- \frac {1}{2}, - \frac {1}{2}, 1\right), B C = \left(- \frac {1}{2}, \frac {1}{2}, 0\right),
> > $$
> >
> > $$
> > B D \cdot B C = \frac {1}{4} - \frac {1}{4} = 0,
> > $$
> >
> > $\therefore BC\perp BD,$ 又:棱台中 $BC//EF,\therefore EF\perp BD$ ;
> >
> > ![](../../../images/questions/part-001/8a0475245b8fc25573412ca87c45537c83e7fff22610ab5f38016244db0eaad6.jpg)
> >
> > [方法三]：三余弦定理法
> >
> > ∵平面 $ACFD_{\perp}$ 平面 ABC，∴ $\cos\angle BCD = \cos\angle ACB\cos\angle ACD = \cos45^{\circ}\cos45^{\circ} = \frac{1}{2}$
> >
...
> > (Ⅱ) [方法一]：几何法
> >
> > 因为 $DF // CH$ ，所以 $DF$ 与平面 $DBC$ 所成角即为与 $CH$ 平面 $DBC$ 所成角.
> >
> > 作 $HG \perp BD$ 于 $G$ ，连接 $CG$ ，由
> > （1）可知， $BC \perp$ 平面 $BHD$
> >
...
> >
> > $$
> > \therefore \sin \angle H C G = \frac {H G}{C H} = \frac {1}{\sqrt {3}} = \frac {\sqrt {3}}{3}
> > $$
> >
> > 故 $DF$ 与平面 $DBC$ 所成角的正弦值为 $\frac{\sqrt{3}}{3}$
> >
> > ![](../../../images/questions/part-001/107a489503f1ef51f7440f7d4b7e43a494e61f267c468f7c2ad21a3e19b7e765.jpg)
> >
> > [方法二]【最优解】：空间向量坐标系法
> >
...
> >
> > $\therefore \left\{ \begin{array}{l} -\frac{1}{2} x - \frac{1}{2} y + z = 0 \\ -\frac{1}{2} x + \frac{1}{2} y = 0 \end{array} \right.$ ，令则，，，，，令 $x = 1$ $y = 1$ $z = 2$ $n = (1,1,1)$
> >
...
> > 以 $\{CH, CB, CD\}$ 为基底，
> >
> > ![](../../../images/questions/part-001/18ae6bd1cac74e5b7d06ed6399f0c07aca65319b86155581eff85ba02ec66785.jpg)
> >
> > 不妨设 $DC = 2BC = 2$ ，则
> >
...
> >
> > 则由 $\left\{\begin{aligned}n\cdot\stackrel{\square\square}{CD}&=0,\\ n\cdot CB&=0,\end{aligned}\right.$ 得 $\left\{\begin{aligned}2x+y+4z&=0,\\ x+y+z&=0,\end{aligned}\right.$ 取 $z=1$ ，得 $n=-3CH+2CB+CD$
> >
> > 设直线 DF 与平面 DBC 所成角为 $\theta$ ，
> >
> > 则直线 $HC$ 与平面 $DBC$ 所成角也为 $\theta$ ，
> >
> > 由公式得 $\sin\theta=\frac{\left|\stackrel{\square\square}{HC}\cdot n_{n}\right|}{\left|HC\right|\left|n\right|}=\frac{2}{\sqrt{2}\cdot\sqrt{6}}=\frac{\sqrt{3}}{3}$
> >
> > [方法四]：三余弦定理法
> >
...
> >
> > 由由（Ⅰ）的结论可得 $\angle BCD = 60^{\circ}$
> >
> > 由三余弦定理，得 $\cos45^{\circ}=\cos30^{\circ}\cdot\cos\theta$
> >
...
> >
> > 【整体评价】（Ⅰ）的方法一使用几何方法证明，方法二利用空间直角坐标系方法，简洁清晰，通性通法确定为最优解；方法三使用了两垂直角的三余弦定理得到 $\angle BCD = 60^{\circ}$ ，进而证明，过程简洁，确定为最优解（Ⅱ）的方法一使用几何做法，方法二使用空间坐标系方法，为通性通法，确定为最优解；方法三使用空间向量的做法，避开了辅助线的求作；方法四使用三余弦定理法，最为简洁，确定为最优解；方法五
> >
> > 采用等体积转化法，避免了较复杂的辅助线·
