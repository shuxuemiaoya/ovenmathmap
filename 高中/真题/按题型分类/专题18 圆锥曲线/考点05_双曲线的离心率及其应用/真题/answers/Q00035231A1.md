...
>
> > [!note]- **【分析】**
> > 【分析】通过向量关系得到 $F_{1}A = AB$ 和 $OA \perp F_{1}A$ ，得到 $\angle AOB = \angle AOF_{1}$ ，结合双曲线的渐近线可得 $\angle BOF_{2} = \angle AOF_{1}, \angle BOF_{2} = \angle AOF_{1} = \angle BOA = 60^{\circ}$ ，从而由 $\frac{b}{a} = \tan 60^{\circ} = \sqrt{3}$ 可求离心率。
>
> > [!note]- **【解析】**
> > 【详解】如图，
