...
> > 所以 $FM=(0,2),FN=(3,4)$ ，
> > 从而可以求得 $FM\cdot FN=0\times3+2\times4=8$ ，
> >
> > 故选 **D**.
>
> > [!tip]- **【名师点睛】**
> > 该题考查的是有关直线与抛物线相交求有关交点坐标所满足的条件的问题，在求解的过程中，首先需要根据题意确定直线的方程，之后需要联立方程组，消元化简求解，从而确定出 $M(1,2), N(4,4)$ ，之后借助于抛物线的方程求得 $F(1,0)$ ，最后一步应用向量坐标公式求得向量的坐标，之后应用向量数量积坐标公式求得结果，也可以不求点 $^{M}$ 、 $^{N}$ 的坐标，应用韦达定理得到结果。
> >
> > $$
> > y ^ {2} = 2 p x (p > 0)
> > $$
